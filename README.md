...
CREATE TABLE tbl_Message (
                             ...
                             FOREIGN KEY (ProfileIDOfReceiver) REFERENCES tbl_Profile(ProfileID)
);

-- Table: Report
CREATE TABLE tbl_Report (
                            ReportID INT IDENTITY(1,1) PRIMARY KEY,
                            ProfileIDOfReporter INT NOT NULL,
                            ProfileIDOfReported INT NOT NULL,
                            TimeOfReport DATETIME2 DEFAULT GETDATE(),
                            FOREIGN KEY (ProfileIDOfReporter) REFERENCES tbl_Profile(ProfileID),
                            FOREIGN KEY (ProfileIDOfReported) REFERENCES tbl_Profile(ProfileID)
);
...
--add logins to a Danfoss Database
                ...
                CREATE USER UserProfile FOR LOGIN UserProfile;
                CREATE USER HRProfile FOR LOGIN HRProfile;
                CREATE USER SAProfile FOR LOGIN SAProfile;

-- Create roles
                CREATE ROLE loginscreen;
                CREATE ROLE ApplicationUser;
                CREATE ROLE HumanResource;
                CREATE ROLE SystemAdmin;

-- Add users to roles
                ALTER ROLE loginscreen ADD MEMBER LoginProfile;
                ALTER ROLE ApplicationUser ADD MEMBER UserProfile;
                ALTER ROLE HumanResource ADD MEMBER HRProfile;
                ALTER ROLE SystemAdmin ADD MEMBER SAProfile;
